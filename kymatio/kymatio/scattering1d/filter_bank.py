...
def calibrate_scattering_filters(J, Q, T, r_psi=math.sqrt(0.5), sigma0=0.1,
    ...
    xi1s, sigma1s, j1 = compute_params_filterbank(sigma_min, Q, r_psi=r_psi,
                                            alpha=alpha)
    xi2s, sigma2s, j2 = compute_params_filterbank(sigma_min, 1, r_psi=r_psi,
                                            alpha=alpha)
...
def scattering_filter_factory(J_support, J_scattering, Q, T, r_psi=math.sqrt(0.5),
    ...
    * k where k is an integer bounded below by 0. The maximal value for k
    ...
    for (xi2, sigma2, j2) in zip(xi2s, sigma2s, j2s):
        ...
        #    eps=eps)
        psi_levels = [morlet_1d(N, xi2, sigma2, normalize=normalize, P_max=P_max,
            eps=eps)]
    ...
    for (xi1, sigma1, j1) in zip(xi1s, sigma1s, j1s):
        ...
        #     P_max=P_max, eps=eps)})
        psi_levels = [morlet_1d(N, xi1, sigma1, normalize=normalize, P_max=P_max, eps=eps)]
    ...
    phi_f = {'levels': phi_levels, 'xi': 0, 'sigma': sigma_low, 'j': 0}
    ...
    t_max_phi = compute_temporal_support(
        phi_f['levels'][0].reshape(1, -1), criterion_amplitude=criterion_amplitude)
