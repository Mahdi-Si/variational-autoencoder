...
class TorchBackend1D(TorchBackend):
    ...
    def fft(cls, x):
        ...
        fft_c = torch.fft.fft(x)
        retval = torch.concat((fft_c.real, fft_c.imag), -1)
        return retval
    ...
    def to_polar(cls, x):
        ...
        x_c = torch.complex(x[..., 0:1], x[..., 1:2])
        phase = torch.angle(x_c)[..., 0:1]
        return mag, phase

    @classmethod
    def to_cartesian(cls, mag, phase):
        # this is not a great real check (shape[-1]=1, so removing for the moment)
        ...
        #
        retval = torch.concat((real, imag), -1)
        return retval
