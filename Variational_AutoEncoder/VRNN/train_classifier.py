...
def extract_latent_representations(vrnn_model, data_loader):
    vrnn_model.eval()
    z_latent_all = []
    targets_all = []
    guid_all = []
    with torch.no_grad():
        for batch_idx, (data, guid, _, target, _) in enumerate(data_loader):
            data = data.to(device)
            results = vrnn_model(data)
            z_latent = torch.stack(results.z_latent, dim=1)  # Shape: [batch_size, seq_len, z_dim]
            z_latent = z_latent.permute(0, 2, 1)  # Shape: [batch_size, z_dim, seq_len]
            z_latent_all.append(z_latent.cpu().numpy())
            targets_reduced = target[:, 0].unsqueeze(1)
            targets_all.append(targets_reduced.cpu().numpy())
            guid_all.append(guid)

    z_latent_all = np.concatenate(z_latent_all, axis=0)
    targets_all = np.concatenate(targets_all, axis=0)
    guid_all = np.concatenate(guid_all, axis=0)

    mask = targets_all != 0
    z_latent_all = z_latent_all[mask.flatten()]
    targets_all = targets_all[mask]
    guid_all = guid_all[mask.flatten()]
    return z_latent_all, targets_all, guid_all
...
if __name__ == '__main__':
    ...
    data_loader = DataLoader(fhr_healthy_dataset, batch_size=batch_size, shuffle=True, num_workers=20)
    ...
    pre_trained_VRNN_model = VRNNGauss(input_dim=input_dim, input_size=raw_input_size, h_dim=h_dim, z_dim=z_dim,
                                       n_layers=n_layers, device=device, log_stat=log_stat, bias=False)

    print(f'VRNN_Model:  \n {pre_trained_VRNN_model}')
    print('==' * 50)
    pre_trained_VRNN_model = pre_trained_VRNN_model.to(device)

    check_point_path = os.path.normpath(
        r"C:\Users\mahdi\Desktop\Mahdi-Si-Projects\AI\runs\variational-autoencoder\VM\2_channelss\VRNN-7129.pth"
    )
    ...
    X_all, y_all, guid_all = extract_latent_representations(pre_trained_VRNN_model, data_loader)
    print('done')

    splits = get_splits(y_all, valid_size=.2, stratify=True, random_state=23, shuffle=True, show_plot=False)

    tfms = [None, [Categorize()]]
    dsets = TSDatasets(X_all, y_all, tfms=tfms, splits=splits, inplace=True)

    dls = TSDataLoaders.from_dsets(dsets.train, dsets.valid, bs=batch_size, batch_tfms=[TSStandardize()], num_workers=0)

    model_inception_time = InceptionTime(dls.vars, dls.c)
    learn_inception_time = Learner(dls, model_inception_time, metrics=accuracy)
    save_path_stage = os.path.join(train_results_dir, 'Inception_Time_stage0')
    learn_inception_time.save(save_path_stage)
    learn_inception_time.load(save_path_stage)
    learn_inception_time.lr_find()
    learn_inception_time.fit_one_cycle(epochs_num, lr_max=1e-3)
    save_path_model_all = os.path.join(model_checkpoint_dir, 'export_inception_time')
    learn_inception_time.save_all(path=save_path_model_all, dls_fname='dls', model_fname='model_inception_time',
                                  learner_fname='learn_inception_time')

    # XCM model---------------------------------------------------------------------------------------------------------
    model_xcm = XCM(c_in=z_dim, c_out=2, seq_len=input_size)
    learn_xcm = ts_learner(dls, model_xcm, metrics=[accuracy])
    save_path_stage = os.path.join(train_results_dir, 'XCM_stage0')
    learn_xcm.save(save_path_stage)
    learn_xcm.load(save_path_stage)
    # learn_xcm.lr_find()
    learn_xcm.fit_one_cycle(epochs_num, lr_max=0.00039)
    save_path_model_all = os.path.join(model_checkpoint_dir, 'export_xcm')
    learn_inception_time.save_all(path=save_path_model_all, dls_fname='dls', model_fname='model_xcm',
                                  learner_fname='learn_xcm')

    # ResCNN model-------------------------------------------------------------------------------------------------
    model_rescnn = ResCNN(c_in=X_all.shape[1], c_out=2)
    learn_rescnn = ts_learner(dls, model_rescnn, metrics=[accuracy])
    save_path_stage = os.path.join(train_results_dir, 'rescnn_stage0')
    learn_rescnn.save(save_path_stage)
    learn_rescnn.load(save_path_stage)
    # learn_rescnn.lr_find()
    learn_rescnn.fit_one_cycle(epochs_num, lr_max=0.00013)
    save_path_model_all = os.path.join(model_checkpoint_dir, 'export_rescnn')
    learn_inception_time.save_all(path=save_path_model_all, dls_fname='dls', model_fname='model_rescnn',
                                  learner_fname='learn_rescnn')
