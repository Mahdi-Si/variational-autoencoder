...
class VRNNGauss(VrnnGaussAbs):
    def __init__(self, input_dim, input_size, h_dim, z_dim, n_layers, device, log_stat, modify_z=None, modify_h=None,
        ...
        # self.rnn = nn.LSTM(self.h_dim + self.h_dim, self.h_dim, self.n_layers, bias=bias, batch_first=True)
    ...
    def forward(self, y):
        ...
        for t in range(y.size(0)):
            ...
            # encoder: y_t, h_t -> z_t
            enc_t = self.enc(torch.cat([phi_y_t, h[-1]], 1))  # (batch_size, input_dim)
            enc_mean_t = self.enc_mean(enc_t)  # (batch_size, latent_dim)
            enc_logvar_t = self.enc_logvar(enc_t)  # (batch_size, latent_dim)

            # prior: h_t -> z_t (for KLD loss)
            prior_t = self.prior(h[-1])  # (batch_size, input_dim)
            prior_mean_t = self.prior_mean(prior_t)  # (batch_size, latent_dim)
            prior_logvar_t = self.prior_logvar(prior_t)  # (batch_size, latent_dim)
            ...
            KLD_2, kld_element_2 = self.kld_gauss_(enc_mean_t, enc_logvar_t, prior_mean_t, prior_logvar_t)
