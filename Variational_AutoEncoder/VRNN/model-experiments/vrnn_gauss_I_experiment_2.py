...
class VRNNGauss(VrnnGaussAbs):
    def __init__(self, input_dim, input_size, h_dim, z_dim, n_layers, device, log_stat, modify_z=None, modify_h=None,
        ...
        self.phi_z = nn.Sequential(
            nn.Linear(self.z_dim, int(3 * self.z_dim / 2)),
            nn.ReLU(),
            nn.Linear(int(3 * self.z_dim / 2), int(9 * self.z_dim / 4)),
            nn.ReLU(),
            nn.Linear(int(9 * self.z_dim / 4), int(self.h_dim / 2))
        )
        ...
        self.dec_mean = nn.Sequential(
            nn.Linear(int(self.h_dim / 2), int(3 * self.h_dim / 8)),
            nn.ReLU(),
            nn.Linear(int(3 * self.h_dim / 8), self.input_dim))
