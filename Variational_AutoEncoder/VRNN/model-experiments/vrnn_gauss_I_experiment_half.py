...
class VRNNGauss(VrnnGaussAbs):
    def __init__(self, input_dim, input_size, h_dim, z_dim, n_layers, device, log_stat, modify_z=None, modify_h=None,
        ...
        self.residual_latent = nn.Sequential(
    ...
    def forward(self, y):
        ...
        for t in range(y.size(0)):
            ...
            # encoder: y_t, h_t -> z_t
            enc_t = self.enc(torch.cat([phi_y_t, phi_h_t], 1)) + self.residual_input(y[t])
            ...
            dec_t = self.dec(phi_z_t) + self.residual_latent(z_t)
